...
import {
  SAVE_ENTRY, GET_ENTRY_LIST, GET_ENTRY, DESTROY_ENTRY,
  CLEAR_ACTIVE_ENTRY, CLEAR_ENTRIES, BULK_DESTROY_ENTRIES, SELECT_ALL_ENTRIES,
  SELECT_NONE_ENTRIES, TOGGLE_SELECT_ENTRY
...
export default handleActions({
  ...
  [`${SAVE_ENTRY}_COMPLETED`]: (state, action) => {
    return state.set('entryDetail', initialEntryDetail.set('data', action.payload._immutableData));
  },
  ...
  [`${GET_ENTRY_LIST}_FAILED`]: (state, action) => {
    return state
      .setIn(['entryList', 'error'], action.payload)
  ...
  [SELECT_ALL_ENTRIES]: (state) => {
    return state.set(
      'selectedEntries', state.getIn(['entryList', 'data', 'results'])
    );
  },
